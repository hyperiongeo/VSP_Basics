...
def q_simulation(VSP, timerange, frange, thead, trace, fs,twin, title_spec,**kwargs):
    ...
    if (twin =='y')or(twin=='Y'):
        ...
        w=sig.windows.tukey(data_trimd.shape[0], alpha=.1)    
    ...
    # for a full trace
        ...
        w=sig.windows.tukey(data_single.shape[0], alpha=.1)    
...
def qest(VSP, thead,fs, **kwargs):
    ...
    if (twin =='y')or(twin=='Y'):
        ...
        # Apply window to segment
        w=sig.windows.tukey(data_trimd_ref.shape[0], alpha=.1)    
    ...
    # for a full trace
        ...
        # Apply window to whole trace
        w=sig.windows.tukey(data_ref.shape[0], alpha=.1)    
