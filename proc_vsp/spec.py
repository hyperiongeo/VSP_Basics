...
import proc_vsp.utils as Utils
...
def spectra(idata, timerange, frange, thead, fs,twin):
    ...
    if twin in ['Y', 'y']:        
        ...
        w=sig.windows.tukey(idata_trimd.shape[1], alpha=.1)    
    ...
    # for a full trace
        ...
        w=sig.windows.tukey(idata.shape[1], alpha=.1)    
    ...
    ####################### do the fft  #####################################
    X = np.zeros(shape = (idata_win.shape[0], N),dtype=np.complex128)
...
'''    
    ...
    print(' data shape :',idata.shape, ' TTobs shape :', TTobs.shape)
...
def simple_bpf(VSPdata, tf1, tf2, tf3, tf4, fs, qc='n'):
