...
# from math import sqrt, sin, cos, radians, degrees, tan, atan
...
#    degs = np.concatenate([deg1,deg2]) # this version produces same order data as
    ...
    for i in range(0,theta.shape[0]):
        phase_velp[i,] = np.sqrt(.5)*np.sqrt((Vpx**2*np.sin(theta[i,])**2+Vpz**2*np.cos(theta[i,])**2 +Vsz**2)+ 
        np.sqrt((( Vpx**2-Vsz**2)*np.sin(theta[i,])**2 +(Vpz**2-Vsz**2)*np.cos(theta[i,])**2)**2 +
        (Vpz**2-Vsz**2)*(Vpn**2-Vpx**2)*(np.sin(2*theta[i,]))**2))

        phase_vels[i,] = np.sqrt(.5)*np.sqrt((Vpx**2*np.sin(theta[i,])**2+Vpz**2*np.cos(theta[i,])**2 +Vsz**2)-
        np.sqrt((( Vpx**2-Vsz**2)*np.sin(theta[i,])**2 +(Vpz**2-Vsz**2)*np.cos(theta[i,])**2)**2 +
        (Vpz**2-Vsz**2)*(Vpn**2-Vpx**2)*(np.sin(2*theta[i,]))**2))
...
def aniso_phase_calc(wvsptzoff,tu,Vpz,Vsz,epsilon,delta, save):
    ...
    f = open(wvsptzoff, 'r', encoding='utf-8')
...
def group_vel(Vpz,Vsz, epsilon, delta,dep):
    ...
    for i in range(0,theta.shape[0]):
        term0[i,] = (Vpx**2-Vpz**2)*np.sin(2*theta[i,])
        term1[i,] = (Vpx**2-Vs[1,]**2)*np.sin(theta[i,])**2+(Vpz**2-Vsz**2)*np.cos(theta[i,])**2        
        term2[i,] = (Vpz**2-Vsz**2)*(Vpn**2-Vpx**2)
        term3[i,] = term0[i,]+1/(np.sqrt(term1[i,]**2+term2[i,]*(np.sin(2*theta[i,])**2)))*\
        (term0[i,]*(Vpx**2-Vpz**2)*np.sin(2*theta[i,])+term0[i,]*np.sin(4*theta[i,]))    
    
        term4[i,] = (1/(4*Vp[i,]))*term3[i,]
        Vp_grp[i,] = np.sqrt(Vp[i,]**2+term4[i,]**2)

        term3_s[i,] = term0[i,]-1/(np.sqrt(term1[i,]**2+term2[i,]*(np.sin(2*theta[i,])**2)))*\
        (term1[i,]*(Vpx**2-Vpz**2)*np.sin(2*theta[i,])+term2[i,]*np.sin(4*theta[i,]))    
        
        term4_s[i,] = (1/(4*Vs[i,]))*term3_s[i,]    
        Vs_grp[i,] = np.sqrt(Vs[i,]**2+term4_s[i,]**2)
        
        # calculate the group angle from phase velocity and phase angle
        Vp_grp_angr[i,] = theta[i,]+np.atan((1/Vp[i,]) * term4[i,])
        Vs_grp_angr[i,] = theta[i,]+np.atan((1/Vs[i,]) * term4_s[i,])

        # calculate wavefront x,z compnents
        Xp_grp[i,] = Vp_grp[i,]*Tp*np.sin(Vp_grp_angr[i,])
        Zp_grp[i,] = Vp_grp[i,]*Tp*np.cos(Vp_grp_angr[i,])

        Xs_grp[i,] = Vs_grp[i,]*Tp*np.sin(Vs_grp_angr[i,])
        Zs_grp[i,] = Vs_grp[i,]*Tp*np.cos(Vs_grp_angr[i,])
        
        # to get the normal moveout curve at fixed reflector depth
        Xp_grp_refl[i,]=dep*np.tan(Vp_grp_angr[i,])
        Zp_grp_refl[i,]=np.sqrt((4*dep**2)+Xp_grp_refl[i,]**2)/Vp_grp[i,]
        
        Xs_grp_refl[i,]=dep*np.tan(Vs_grp_angr[i,])
        Zs_grp_refl[i,]=np.sqrt((4*dep**2)+Xs_grp_refl[i,]**2)/Vs_grp[i,]
